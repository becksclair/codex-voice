...
`codex-voice-tts` implements text-to-speech synthesis with Google Gemini TTS and ElevenLabs backends. It loads strict versioned config from the platform config directory (`~/.config/codex-voice/config.json` on Linux), resolves voices, orchestrates ordered backend fallback, and converts provider-native output to the requested format.
...
- Config loading and voice resolution: `src/config/` (`VoiceConfigLoader`, `ResolvedTtsConfig`).
...
- Secret resolution: `src/secret.rs` (`resolve_provider_key`).
...
rg -n "VoiceConfigLoader|ResolvedTtsConfig|ProviderKind|provider_order" src/config/
...
- The version-1 config is strict and uses an ordered backend list per voice. Do not add legacy readers, partial overlays, inline secrets, or silent coercion.
